...
const projectId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
...
const staticBirdData = [
  {
    ...
    imageUrl: 'https://images.unsplash.com/photo-1518979293113-500b6f4a5455'
  },
  {
    ...
    imageUrl: 'https://images.unsplash.com/photo-1559632873-5426a5746353'
  },
  {
    name: 'Blue Jay',
    description: 'A noisy, intelligent bird with a prominent crest and vibrant blue, black, and white plumage. They are known for mimicking the calls of other birds.',
    imageUrl: 'https://images.unsplash.com/photo-1628522399698-b6b80308a3f8'
  },
  {
    name: 'House Sparrow',
  ...
  },
  {
    ...
    imageUrl: 'https://images.unsplash.com/photo-1620588151433-438a0352780e'
  },
  {
    name: 'Mourning Dove',
...
// Header Component
const Header = ({ setActivePage, setSelectedBird }) => {
...
// Explore Birds Page
const ExploreBirds = ({ setSelectedBird }) => {
  ...
  const handleGuess = async () => {
    ...
    try {
      ...
      const matchedBirds = birdGuesses
        .map(guess => staticBirdData.find(b => b.name.toLowerCase().includes(guess.toLowerCase())))
...
const MySightings = ({ db, userId, projectId }) => {
...
const PublicSightings = ({ db, projectId }) => {
  const [sightings, setSightings] = useState([]);
  const [newSightingBird, setNewSightingBird] = useState('');
  const [newSightingNotes, setNewSightingNotes] = useState('');
  const [isLoading, setIsLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      sightingsData.sort((a, b) => new Date(b.dateSeen) - new Date(a.dateSeen));
      setSightings(sightingsData);
      setIsLoading(false);
    }, (error) => {
